...
TABLE_COUNT=$(mysql --skip-ssl -h "$DB_HOST_VAL" -P "$DB_PORT_VAL" -u "$DB_USER_VAL" -p"$DB_PASS_VAL" -e "SELECT count(*) FROM information_schema.tables WHERE table_schema = '$DB_NAME_VAL';" -N 2>/dev/null || echo 0)
...
if [ "$TABLE_COUNT" -eq 0 ]; then
    ...
    if [ -f /var/www/html/database_export.sql ]; then
        mysql --skip-ssl -h "$DB_HOST_VAL" -P "$DB_PORT_VAL" -u "$DB_USER_VAL" -p"$DB_PASS_VAL" "$DB_NAME_VAL" < /var/www/html/database_export.sql
