...
<style>
    /* Warna Tema Emas/Warning Gallery Puan */
    :root {
        --gold-primary: #c49a45;
        --gold-hover: #b38a36;
        --light-bg: #fdfcf9;
    ...
    /* Ubah warna teks & ikon saat aktif */
...
<div class="container py-5">
    <div class="row mb-4">
        <div class="col-12 d-flex align-items-center">
            ...
            <div>
                <h2 class="fw-bold mb-0 text-dark" style="font-family: 'Playfair Display', serif;">Pesanan Saya</h2>
                <p class="text-muted mb-0 small">Pantau seluruh transaksi dan status pesanan Anda</p>
            </div>
        </div>
    </div>

    <div class="custom-tabs-container mb-4">
        <a href="{{ route('orders.index') }}" class="custom-tab-item {{ request('status') == null ? 'active' : '' }}">
            <i class="bx bx-grid-alt fs-4 text-muted"></i>
            <div class="tab-text-wrapper">
                <span class="tab-title">Semua</span>
                <span class="tab-subtitle">Lihat seluruh pesanan</span>
            </div>
        </a>

        <a href="{{ route('orders.index', ['status' => 'unpaid']) }}" class="custom-tab-item {{ request('status') == 'unpaid' ? 'active' : '' }}">
        ...
        <a href="{{ route('orders.index', ['status' => 'dikemas']) }}" class="custom-tab-item {{ request('status') == 'dikemas' ? 'active' : '' }}">
            <i class="bx bx-package fs-4 text-muted"></i>
            <div class="tab-text-wrapper">
                <span class="tab-title">Sedang Dikemas</span>
                <span class="tab-subtitle">Pesanan sedang dikemas</span>
            </div>
        </a>

        <a href="{{ route('orders.index', ['status' => 'dikirim']) }}" class="custom-tab-item {{ request('status') == 'dikirim' ? 'active' : '' }}">
            <i class="bx bx-truck fs-4 text-muted"></i>
            <div class="tab-text-wrapper">
                <span class="tab-title">Dikirim</span>
                <span class="tab-subtitle">Pesanan dalam perjalanan</span>
            </div>
        </a>

        <a href="{{ route('orders.index', ['status' => 'selesai']) }}" class="custom-tab-item {{ request('status') == 'selesai' ? 'active' : '' }}">
            <i class="bx bx-check-circle fs-4 text-muted"></i>
            <div class="tab-text-wrapper">
                <span class="tab-title">Selesai</span>
                <span class="tab-subtitle">Pesanan telah selesai</span>
            </div>
        </a>

        <a href="{{ route('orders.index', ['status' => 'dibatalkan']) }}" class="custom-tab-item {{ request('status') == 'dibatalkan' ? 'active' : '' }}">
            <i class="bx bx-x-circle fs-4 text-muted"></i>
            <div class="tab-text-wrapper">
                <span class="tab-title">Dibatalkan</span>
                <span class="tab-subtitle">Pesanan dibatalkan</span>
            </div>
        </a>
    </div>
                            ...
                            <div class="loc-header d-flex justify-content-between align-items-center">
                                ...
                                <div class="loc-status">
                                    @if($order->status == 'cancelled')
                                        <span class="badge bg-dark px-3 py-2 rounded-pill">Dibatalkan</span>
                                    @elseif($isUnpaid && $isExpired)
                                        <span class="badge bg-secondary px-3 py-2 rounded-pill">Kedaluwarsa</span>
                                    @elseif($isUnpaid)
                                        <span class="badge bg-danger px-3 py-2 rounded-pill shadow-sm">Belum Bayar</span>
                                    @elseif($order->status == 'processing')
                                        <span class="badge bg-info text-dark px-3 py-2 rounded-pill">Diproses</span>
                                    @elseif($order->status == 'packaging' || $order->status == 'confirmed')
                                        <span class="badge bg-primary px-3 py-2 rounded-pill">Dikemas</span>
                                    @elseif($order->status == 'delivered' || $order->status == 'shipped')
                                        <span class="badge bg-warning text-dark px-3 py-2 rounded-pill">Dikirim</span>
